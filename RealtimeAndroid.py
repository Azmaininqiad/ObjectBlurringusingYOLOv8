...
subprocess.Popen(['scrcpy', '--no-control', '--max-size', '800'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

# Wait for scrcpy to start
import time
time.sleep(2)

# Find the scrcpy window and capture its dimensions
import pygetwindow as gw
window = gw.getWindowsWithTitle('scrcpy')[0]
window.moveTo(0, 0)
